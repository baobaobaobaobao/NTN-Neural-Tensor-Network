...
def inference(batch_placeholders, corrupt_placeholder, init_word_embeds, entity_to_wordvec,\
    ...
    # for i in entity_to_wordvec:
    #     print(type(i),i)
    #     print(tf.constant(i)-1)
    #     exit()
    ...
    for r in range(num_relations):
        ...
        e1, e2, e3 = tf.split(tf.cast(batch_placeholders[r], tf.int32), 3, 1)#change
        e1v = tf.transpose(tf.squeeze(tf.gather(entEmbed, e1, name='e1v'+str(r)),[1]))      #tf.squeeze delete the shape==1    
        ...
        print(preactivation_pos)
        preactivation_pos = tf.stack(preactivation_pos) #change
        preactivation_neg = tf.stack(preactivation_neg) #change
        print(preactivation_pos)
        exit()
        temp2_pos = tf.matmul(V[r], tf.concat([e1v_pos, e2v_pos],0))#change
        temp2_neg = tf.matmul(V[r], tf.concat([e1v_neg, e2v_neg],0))#change
        ...
        if not is_eval:
            predictions.append(tf.stack([score_pos, score_neg]))  #change
        else:
            predictions.append(tf.stack([score_pos, tf.reshape(label_placeholders[r], num_rel_r)]))  #change
        #print("score_pos_and_neg: "+str(predictions[r].get_shape()))


    #print("Concating predictions")
    predictions = tf.concat(predictions,1) #change
...
def loss(predictions, regularization):
    ...
    temp1 = tf.maximum(tf.subtract(predictions[1, :], predictions[0, :]) + 1, 0)#change sub->subtract
...
def eval(predictions):
    ...
    inference, labels = tf.split(predictions, 2, 0)
